...
class TorchSMPL4Garment(nn.Module):
    def __init__(self, gender):
        ...
        model = np.load(os.path.join(ROOT, 'smpl/smpl_hres_{}.npz'.format(gender))) # smpl_hres_{}.npz'.format(gender)))
        with open(os.path.join(ROOT, 'garment_class_info.pkl'), 'rb') as f:
            class_info = pickle.load(f, encoding='latin-1')
        for k in class_info.keys():  #class info has the I(m,n), which means the garment vertex associated with body shape vertex 
        ...
        np_v_template = np.array(model['v_template'], dtype=np.float) #resolution is higher (27554, 3)
    ...
    def forward(self, theta, beta=None, garment_d=None, garment_class=None, rotate_base=False):
        ...
        T = torch.matmul(W, A.view(num_batch, 24, 16)).view(num_batch, -1, 4, 4)  #batch_nb,vertex_nb,4,4
        ...
        if garment_class is not None and garment_d is not None:
            ...
            v_deformed = v_deformed[:, vert_indices] + garment_d # nb_batch,2271,3
